...
public class LeftDoorScriptMiddleRight : MonoBehaviour
{
    ...
    void Move()
    {
        if (time2 > 0.3)
        {
            if (!MoveFinished)
            {
                if (transform.position.x > 11.23 + Parent.transform.position.x)
                {
                ...
                if (transform.position.x < 11.93 + Parent.transform.position.x)
                {
