...
public class LeftDoorScriptMiddleLeft : MonoBehaviour
{
    ...
    void Move()
    {
        if (time2 > 0.3)
        {
            if (!MoveFinished)
            {
                if (transform.position.x > -8.83 + Parent.transform.position.x)
                {
                ...
                if (transform.position.x < -8.13 + Parent.transform.position.x)
                {
